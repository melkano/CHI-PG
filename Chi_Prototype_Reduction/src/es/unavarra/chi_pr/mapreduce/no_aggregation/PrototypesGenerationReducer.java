...
public class PrototypesGenerationReducer extends Reducer<ByteArrayWritable, LongArrayWritable, Text, NullWritable> {
	 ...
	 * @param minNumExamples if the highest number of examples is less than minNumExamples, the value returned will be -1
	 * @return class with the highest number of examples
	 */
	private byte getMajorityClass(long[] numExamples, int minNumExamples){
		
		majClass = -1;
		
		numExClass = minNumExamples;
		
		for (i = 0; i < numExamples.length; i++)
			if (numExamples[i] > numExClass){
				numExClass = numExamples[i];
				majClass = (byte)i;
			}
			else if (numExamples[i] == numExClass && numExClass > 0){
				// In case of a tie, get the minority class
				if (majClass < 0 || Mediator.getClassNumExamples()[i] < Mediator.getClassNumExamples()[majClass]){
					numExClass = numExamples[i];
					majClass = (byte)i;
				}
			}
    ...
    public void reduce(ByteArrayWritable key, Iterable<LongArrayWritable> values, Context context) throws IOException, InterruptedException {
    	...
    	// Select the class with highest number of examples
    	majClass = getMajorityClass(numExamples, Mediator.getPrototypeMinNumExamples());
    	if (majClass > -1){
