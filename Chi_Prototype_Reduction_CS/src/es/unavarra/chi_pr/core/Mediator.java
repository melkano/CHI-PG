...
public class Mediator {
	...
	/**
	 * Number of examples and frequency of each class field
	 */
	private static final String CLASS_NUM_EXAMPLES_FREQ_FIELD = "class_num_examples_freq";
     ...
     */
    private static double[] classFreq;
    ...
    /**
     * Minimum number of examples per prototype for each class
     */
    private static int[] protoMinNumExClass;
    ...
    /**
     * Returns the frequency ([0-1]) of each class
     * @return frequency ([0-1]) of each class
     */
    public static double[] getClassFreq (){
        return classFreq;
    }
    ...
    /**
     * Returns the minimum number of examples required to generate a prototype of the given class
	 * @return the minimum number of examples required to generate a prototype of the given class
	 */
	public static int getPrototypeMinNumExamples(int classIndex) {
		return protoMinNumExClass[classIndex];
	}
	
	/**
     * Returns the minimum number of examples required to generate a prototype (specified by the user)
	 * @return the minimum number of examples required to generate a prototype (specified by the user)
	 */
    ...
    public static void readConfiguration () throws Base64DecodingException, IOException, ClassNotFoundException{
    	...
    	// Read the number of examples and the frequency of each class
    	bytes = Base64.decode(configuration.get(CLASS_NUM_EXAMPLES_FREQ_FIELD).getBytes());
    	objectInputStream = new ObjectInputStream(new ByteArrayInputStream(bytes));
    	classNumExamples = (long[])objectInputStream.readObject();
    	classFreq = (double[])objectInputStream.readObject();
    	
    	// Compute the minimum number of examples per prototype for each class
    	protoMinNumEx = Integer.parseInt(configuration.get(PROTO_MIN_NUM_EX_FIELD));
    	protoMinNumExClass = new int[numClassLabels];
    	for (int classIndex = 0; classIndex < numClassLabels; classIndex++)
    		protoMinNumExClass[classIndex] = (int)Math.round(protoMinNumEx * numClassLabels * classFreq[classIndex]);
    ...
    public static void readHeaderFile (String filePath) throws IOException, URISyntaxException{
        ...
        saveClassNumExamplesAndFreq(classNumExamples);
    ...
    /**
     * Stores the number of examples and the frequency of each class in the configuration file
     * @param numExamplesByClass number of examples of each class
     * @throws IOException 
     */
    private static void saveClassNumExamplesAndFreq (ArrayList<Long> numExamplesByClass) throws IOException {
    	
    	// Convert input ArrayList into an array
    	classNumExamples = new long[numExamplesByClass.size()];
    	for (int i = 0; i < numExamplesByClass.size(); i++)
    		classNumExamples[i] = numExamplesByClass.get(i);
    	
    	// Compute the frequency of each class
    	long sum = 0;
    	for (int i = 0; i < classNumExamples.length; i++)
    		sum += classNumExamples[i];
    	classFreq = new double[classNumExamples.length];
    	for (int i = 0; i < classFreq.length; i++)
    		classFreq[i] = (double)classNumExamples[i] / sum;
    	
    	// Convert both arrays into a string
    	ByteArrayOutputStream byteArrayOutputStream = new ByteArrayOutputStream();
	    ObjectOutputStream objectOutputStream = new ObjectOutputStream(byteArrayOutputStream);
	    objectOutputStream.writeObject(classNumExamples);
	    objectOutputStream.writeObject(classFreq);
	    objectOutputStream.close();
	    String outputString = new String(Base64.encode(byteArrayOutputStream.toByteArray()));
	    
	    configuration.set(CLASS_NUM_EXAMPLES_FREQ_FIELD,outputString);
